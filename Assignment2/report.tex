...
Implementing the negative selection algorithm from scratch would be challenging for several reasons. One of the biggest obstacles in its implementation would be generation of the detectors. The detectors have to be checked against the entire training set, seeing if they do not match any of the normal strings. Because the algorithm needs to compare a large number of strings against many detectors, it makes the algorithm slower as the number of comparisons grows quickly. Efficient data structures and optimization would be necessary to keep the runtimes low. 

Another obstacle would be the selection of the right parameters, such as chunk size $n$ and $r$. As demonstrated by this project, different parameter values can lead to different results, both performance and runtime wise, and some parameter configurations cause the algorithm to fail entirely. During the exeperiments, it was observed that runs with the same parameter configurations could take differently long, which would make performance tuning and debugging much more difficult.
