...
\begin{table}[h]
\centering
\caption{Optimized validation across both datasets (Chunked Training, $n=7$, $r=5$)}
\begin{tabular}{l c c c}
\hline
\textbf{Dataset} & \textbf{Split 1} & \textbf{Split 2} & \textbf{Split 3} \\
\hline
\texttt{snd-cert} & 0.9774 & 0.9841 & 0.9839 \\
\texttt{snd-unm}  & 0.9804 & 0.9821 & 0.9846 \\
\hline
\end{tabular}
\end{table}
...
\subsection{Numerical Instability at Low $r$}

As we could repeatedly see with the results, running the tests with $n=15$ and $r<5$ as parameters produces NaN values. \texttt{Negsel2} computes detector counts analytically using an inclusion-exclusion formula over large combinatorial terms. With $n=15$ and small $r$, individual terms in this alternating sum reach magnitudes on the order of $10^{18}$. Subtracting these nearly-equal large values causes catastrophic floating-point errors, producing negative counts (e.g., $-7.42 \times 10^{18}$, observed when running it without the -l flag). Since the \texttt{-l} flag computes $\log_2(1+x)$ of these counts, taking the logarithm of a negative number yields NaN. For $r \geq 5$, the r-gram space is large enough that the combinatorial terms remain numerically stable and we can get useable results.


\subsection{Relationship Between Chunk Size and AUC}
An unexpected finding from the parameter sweep is the relationship between chunk size $n$ and AUC: $n=7$ and $n=15$ both achieve AUC = 0.9900 at $r=5$, while $n=10$ plateaus at 0.9804. We believe that this can be explained by the competing mechanisms through which each chunk size achieves discrimination:


Small chunks ($n=7$) are granular. Each 1000-character sequence leads to about 142 chunks. The algorithm is highly selective at $n=7$ and $r=5$, and the large number of chunks increases the chance of detecting localized anomalies.

Large chunks ($n=15$) are informative. Longer chunks carry more contextual information, making normal patterns inherently more distinctive from anomalous ones. The chunks themselves are informative enough to separate the two classes.

Medium chunks of $n=10$ fall in between these two values and instead of combining the strengths of small and large chunks, they seem to not do either thing as well. The comparatively bigger chunks when contrasted with $n=7$ lead to fewer samples, while the comparatively smaller chunks when contrasted with $n=15$ provide less information for distinguishing between normal and anomalous test chunks.
