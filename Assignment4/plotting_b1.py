...
def plot_fitness_curves(results, title="Best fitness over generations"):
    """Plot best fitness per generation as median with 95% CI band."""
    fig, ax = plt.subplots(figsize=(10, 5))

    # Pad all histories to the same length (hold final value for early finishers)
...
def plot_beeswarm(data_dict, G_max, title="t_finish distribution (beeswarm)"):
    ...
    None values (target not found) are plotted at G_max with upward triangles
    to indicate they were capped.

    data_dict: {label: [t_finish or None, ...]}
    """
    fig, ax = plt.subplots(figsize=(8, 5))
    labels = list(data_dict.keys())
    _added_found_label = False
    _added_capped_label = False
    for i, label in enumerate(labels):
        ...
        lbl = "Found" if not _added_found_label and found else None
        if found:
            _added_found_label = True
        ax.scatter(
            [i] * len(found) + jitter[:len(found)], found,
            alpha=0.7, s=50, zorder=3, color="C0", label=lbl,
        )
        if not_found_count > 0:
            jitter_nf = np.random.normal(0, 0.05, not_found_count)
            lbl_nf = f"Not found (capped at {G_max})" if not _added_capped_label else None
            _added_capped_label = True
            ax.scatter(
                [i] * not_found_count + jitter_nf, [G_max] * not_found_count,
                alpha=0.8, s=60, marker="^", color="red", zorder=3, label=lbl_nf,
            )
...
def _plot_mu_sweep_single(ax, sweep_results, G_max, L, color="C0", label=None):
    """Helper: scatter individual runs on a mu-sweep axis, marking capped values."""
    ...
    for mu in mus:
        ...
        jitter = np.random.normal(0, (mus[1] - mus[0]) * 0.06, len(found)) if len(mus) > 1 else 0
        if found:
            ax.scatter(
                mu + jitter[:len(found)] if hasattr(jitter, '__len__') else [mu] * len(found),
                found, alpha=0.5, s=30, color=color, zorder=3,
            )
        if n_capped > 0:
            jitter_c = np.random.normal(0, (mus[1] - mus[0]) * 0.06, n_capped) if len(mus) > 1 else 0
            ax.scatter(
                mu + jitter_c[:n_capped] if hasattr(jitter_c, '__len__') else [mu] * n_capped,
                [G_max] * n_capped, alpha=0.7, s=40, marker="^", color="red", zorder=3,
            )

    # Plot means as connected line
    means = []
    for mu in mus:
        vals = [v if v is not None else G_max for v in sweep_results[mu]]
        means.append(np.mean(vals))
    ax.plot(mus, means, marker="o", markersize=6, color=color, linewidth=1.5,
            alpha=0.8, label=label or "Mean", zorder=4)


def plot_mu_vs_tfinish(sweep_results, G_max, L, K_label="K=2"):
    """
    Plot mu vs t_finish from a sweep experiment.
    Shows individual runs as scatter + mean line. Capped values shown as red triangles.
    """
    fig, ax = plt.subplots(figsize=(10, 5))
    mus = sorted(sweep_results.keys())
    mu_labels = [f"{mu*L:.1f}/L" for mu in mus]

    _plot_mu_sweep_single(ax, sweep_results, G_max, L, color="C0", label=f"Mean ({K_label})")

    ax.axhline(G_max, color="red", linestyle="--", alpha=0.3, label=f"G_max={G_max}")
    # Add a single triangle to legend
    ax.scatter([], [], marker="^", color="red", s=40, label=f"Capped at G_max")
...
def plot_mu_sweep_comparison(results_k2, results_k5, G_max, L):
    """Plot mu vs t_finish for K=2 and K=5 with individual scatter + mean lines."""
    fig, ax = plt.subplots(figsize=(10, 5))

    _plot_mu_sweep_single(ax, results_k2, G_max, L, color="C0", label="K=2 (mean)")
    _plot_mu_sweep_single(ax, results_k5, G_max, L, color="C1", label="K=5 (mean)")
