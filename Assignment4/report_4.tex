...
\subsubsection*{B1}
\todo[inline]{Add a short paragraph introducing B1: what question this subsection answers (``Does the mutation rate $\mu$ trade off convergence speed against diversity, and does tournament size $K$ change the optimum?''). Currently the reader lands straight on a figure.}
...
\begin{figure}[H]
	...
	\label{fig:b1output2}
\todo[inline]{B.1 Q2 explicitly asks for a beeswarm of $t_{\text{finish}}$ distributions across the 10 runs per $\mu$. If this figure is a fitness curve (as the caption suggests), ADD a second panel / figure with the beeswarm of $t_{\text{finish}}$ (censored at $G_{\max}$ shown as e.g. hollow points). The assignment is specific about the plot type.}
\end{figure}
\begin{figure}[H]
	\centering
...
Comparing the MA and simple EA on the same number of generations is not a fair baseline. Each MA generation includes a full 2-opt local search over every individual in the population, making it far more computationally expensive than a single EA generation. Any advantage observed per generation could therefore reflect additional computation rather than algorithmic superiority. We considered normalising by number of fitness evaluations, but since 2-opt operates within a solution's neighbourhood its evaluation count is not directly comparable to a full population generation in the EA. We therefore compare on wall-clock time, which — while sensitive to system-level factors — provides a practical performance baseline when averaged over 10 runs.
...
\subsection*{Part A}
\subsection*{A1}
\todo[inline]{PLACEHOLDER. The assignment explicitly requires: (i) a table with $f(x)$ and selection probability for $x\in\{2,3,4\}$ for each of the four functions, and (ii) four pie charts (``wheel of fortune''), one per function. Produce both and include them here. Without them this exercise is effectively unanswered.}
